...
public class DashboardBookList extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DashboardBookList frame = new DashboardBookList();
	...
	public DashboardBookList() {
		...
		lblAuthorList.setIcon(new ImageIcon(DashboardBookList.class.getResource("/Images/icons8-open-book-50.png")));
		...
		JLabel id = new JLabel("SEARCH TITLE:");
		id.setFont(new Font("Times New Roman", Font.BOLD, 16));
		id.setBounds(20, 100, 125, 30);
		contentPane.add(id);
		...
		btnRefresh.setBounds(550, 130, 120, 30);
		...
		chooseDate = new JComboBox(new Object[]{});
		chooseDate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				String date = chooseDate.getSelectedItem().toString();
	...
	public void showTable(String _book) {
		...
		String[] colNames = {"ID", "ISBN", "ACCESSION NO.", "TITLE", "COPYRIGHT", "PRICE"};
		...
		for(int i = 0; i < bookList.size(); i++) {
			...
			rows[i][4] = bookList.get(i).getCopyright();
			rows[i][5] = bookList.get(i).getPrice();
